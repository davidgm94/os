...
pub const Driver = extern struct
{
    ...
    pub fn parse_tables(self: *@This()) void
    {
        ...
        const sdt = blk:
        {
            if (@intToPtr(?*RootSystemDescriptorPointer, kernel.address_space.map_physical(arch.FindRootSystemDescriptorPointer(), 16384, Region.Flags.empty()))) |rsdp|
            {
                self.rsdp = rsdp;
                ...
                break :blk @intToPtr(*align(1) DescriptorTable, kernel.address_space.map_physical(sdt_physical_address, 16384, Region.Flags.empty()));
            }
            else
            {
        ...
        while (i < table_count) : (i += 1)
        {
            ...
            const header = @intToPtr(*align(1) DescriptorTable, kernel.address_space.map_physical(address, @sizeOf(DescriptorTable), Region.Flags.empty()));

            switch (header.signature)
            {
                MADT_signature =>
                {
                    const madt_header = @intToPtr(*align(1) DescriptorTable, kernel.address_space.map_physical(address, header.length, Region.Flags.empty()));
                    madt_header.check();
                ...
                FADT_signature =>
                {
                    const fadt = @intToPtr(*align(1) DescriptorTable, kernel.address_space.map_physical(address, header.length, Region.Flags.empty()));
                    fadt.check();
                    ...
                    _ = kernel.address_space.free(@ptrToInt(fadt), 0, false);
                },
                HPET_signature =>
                {
                    const hpet = @intToPtr(*align(1) DescriptorTable, kernel.address_space.map_physical(address, header.length, Region.Flags.empty()));
                    hpet.check();

                    const header_bytes = @ptrCast([*]u8, header);
                    if (header.length > 52 and header_bytes[52] == 0)
                    {
                        if (@intToPtr(?[*]volatile u64, kernel.address_space.map_physical(@ptrCast(*align(1) u64, &header_bytes[44]).*, 1024, Region.Flags.empty()))) |HPET_base_address|
                        {
                    ...
                    _ = kernel.address_space.free(@ptrToInt(hpet), 0, false);
                },
                else => {},
            }
    ...
    {
        return kernel.address_space.map_physical(physical_address, length, Region.Flags.from_flag(.not_cacheable));
    }
};
