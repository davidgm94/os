...
pub fn init() void
{
    // initialize core and kernel memory space
    {
        ...
        Kernel.memory_space.free_regions_base.insert(&region.more.non_core.item_base, region, region.base_address, .panic) catch unreachable;
        Kernel.memory_space.free_regions_size.insert(&region.more.non_core.item.size, region, region.page_count, .allow) catch unreachable;
    }
...
pub const Space = struct
{
    ...
    pub fn map_physical(self: *Space, asked_offset: u64, asked_byte_count: u64, caching: u32) ?u64
    {
        ...
        {
            self.handle_page_fault(region.base_address + page_i * Kernel.Arch.Page.size, 0) catch unreachable;
        }
    ...
    pub fn handle_page_fault(self: *Space, fault_address: u64, fault_flags: u32) HandlePageFaultError!void
    {
        ...
        const region = blk:
        {
            ...
            const found_region = self.find_region(address) orelse
            {
                return HandlePageFaultError.region_not_found;
            };
        ...
        else if (region.flags & (1 << @enumToInt(Memory.Region.Flags.normal)) != 0)
        {
            if (region.flags & (1 << @enumToInt(Memory.Region.Flags.no_commit_tracking)) == 0)
            {
                // @TODO: further check
                 ...
                 {
                     return HandlePageFaultError.outside_commit_range;
                 }
            }

            Kernel.Arch.Page.map(self, Kernel.physical_memory_allocator.allocate(need_zero_pages, null, null, null), address, flags) catch unreachable;
            if (zero_page)
            {
    ...
    pub fn find_region(self: *Self, address: u64) ?*Region
    {
        // @Spinlock
    ...
    pub fn reserve(self: *Self, byte_count: u64, flags: u32, maybe_forced_address: ?u64, maybe_generate_guard_pages: ?bool) ?*Region
    {
        ...
        {
            if (@ptrToInt(self) == @ptrToInt(&Kernel.core_memory_space))
            {
            ...
            else
            {
                ...
                if (self.free_regions_size.find(total_needed_page_count, .smallest_above_or_equal)) |item|
                {
                    const region = item.value.?;
                    self.free_regions_base.remove(&region.more.non_core.item_base);
                    self.free_regions_size.remove(&region.more.non_core.item.size);
                    ...
                    if (region.page_count > total_needed_page_count)
                    {
                        ...
                        self.free_regions_base.insert(&split.more.non_core.item_base, split, split.base_address, .panic) catch unreachable;
                        self.free_regions_size.insert(&split.more.non_core.item.size, split, split.page_count, .allow) catch unreachable;
                    }
                    ...
                    self.used_regions.insert(&region.more.non_core.item_base, region, region.base_address, .panic) catch unreachable;
        ...
        Kernel.Arch.commit_page_tables(self, output_region) catch
        {
            self.unreserve(output_region, false, null);
            return null;
        };
    ...
    fn unreserve(self: *Self, region_to_remove: *Memory.Region, unmap_pages: bool, maybe_guard_region: ?bool) void
    {
        ...
        {
            self.used_regions.remove(&region_to_remove.more.non_core.item_base);

            const address = region_to_remove.base_address;

            if (self.free_regions_base.find(address, .largest_below_or_equal)) |before|
            {
                ...
                if (before_region.base_address + before_region.page_count * Kernel.Arch.Page.size == region_to_remove.base_address)
                {
                    ...
                    self.free_regions_base.remove(before);
                    self.free_regions_size.remove(&before_region.more.non_core.item.size);
                    Kernel.core_heap.free(@ptrToInt(before_region), @sizeOf(Region));
                }
            }
            if (self.free_regions_base.find(address, .smallest_above_or_equal)) |after|
            {
                ...
                if (region_to_remove.base_address + region_to_remove.page_count * Kernel.Arch.Page.size == after_region.base_address)
                {
                    ...
                    self.free_regions_base.remove(after);
                    self.free_regions_size.remove(&after_region.more.non_core.item.size);
                    Kernel.core_heap.free(@ptrToInt(after_region), @sizeOf(Region)); 
                }
            }

            self.free_regions_base.insert(&region_to_remove.more.non_core.item_base, region_to_remove, region_to_remove.base_address, .panic) catch unreachable;
            self.free_regions_size.insert(&region_to_remove.more.non_core.item.size, region_to_remove, region_to_remove.page_count, .allow) catch unreachable;
        }
    }
    ...
    pub fn standard_allocate(self: *Self, byte_count: u64, flags: u32, maybe_base_address: ?u64, maybe_commit_all: ?bool) ![]u8
    {
        ...
        if (commit_all)
        {
            Kernel.physical_memory_allocator.commit_range(self, region, 0, region.page_count) catch
            {
                self.unreserve(region, false, null);
    ...
    pub fn free(self: *Self, address: u64, maybe_expected_size: ?u64, maybe_user_only: ?bool) FreeError!void
    {
        ...
        self.unreserve(region, unmap_pages, null);
...
pub const Heap = struct
{
    ...
    pub fn free(self: *Self, address: u64, maybe_expected_size: ?u64) void
    {
        ...
        if (region.union1.size == 0)
        {
            ...
            // this can be buggy
            Kernel.memory_space.free(@ptrToInt(region), null, null) catch unreachable;
            return;
        }
...
pub const Physical = struct
{
    ...
    pub const Allocator = struct
    {
        ...
        pub fn allocate(self: *Allocator, flags: u32, maybe_count: ?u64, maybe_alignment: ?u64, maybe_below: ?u64) u64
        {
            ...
            if (!self.page_frame_db_initialized)
            {
                if (!simple)
                {
                    Kernel.Arch.CPU_stop();
            ...
            else
            {
                ...
                var page = self.first_zeroed_page;
                if (page == 0)
                {
                    page = self.first_free_page;
                    not_zeroed = true;
                }
                if (page == 0)
                {
                    page = self.last_standby_page;
                    not_zeroed = true;
                }

                if (page != 0)
                {
                    var frame = &self.page_frames[page];
        ...
        pub fn free(self: *Self, asked_page: u64, maybe_mutex_already_acquired: ?bool, maybe_page_count: ?u64) void
        {
            ...
            // @Spinlock
            //
            if (!self.page_frame_db_initialized) Kernel.Arch.CPU_stop();

            const page = asked_page >> Kernel.Arch.Page.bit_count;

            self.insert_free_pages_start();

            for (self.page_frames[page..page_count + page]) |*frame|
            {
                if (frame.state == .free) Kernel.Arch.CPU_stop();

                if (self.commit_fixed_limit != 0) self.active_page_count -= 1;

                self.insert_free_pages_next(page);
            }

            self.insert_free_pages_end();

            // @Spinlock
        }

        fn insert_free_pages_start(self: *Self) void
        {
            _ = self;
        }

        fn insert_free_pages_end(self: *Self) void
        {
